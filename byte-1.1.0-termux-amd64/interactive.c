...
static void le_draw(LE *e) {
    ...
    if (e->plen + e->pos > 0) {
        char s[32]; snprintf(s,sizeof(s),"\r\033[%dC", e->plen+e->pos);
...
static void le_ins(LE *e, char c) {
    ...
    e->buf[e->pos++]=c; e->len++; e->buf[e->len]='\0'; le_draw(e);
}
...
/* پاک کردن پیام خطا - فقط بخش بعد از ']' رو نشون بده */
static const char *clean_err(const char *msg) {
    if (!msg) return "unknown error";
    const char *p = strstr(msg, "]:"); /* [string ...]:line: msg */
    if (p) {
        p += 2;
        while (*p && *p != ' ' && *p != ':') p++;
        if (*p == ':') p++;
...
static void exec(const char *code) {
    ...
    if(luaL_loadstring(L,expr)==LUA_OK) {
        ...
        /* خطای runtime از expression */
        const char *err = lua_tostring(L,-1);
        fprintf(stderr, RED"Error: "RESET"%s\n", clean_err(err));
        lua_settop(L,0); return;
    } else lua_pop(L,1);

    /* اجرا به عنوان statement */
...
static void repl(void) {
    ...
    for(;;) {
        ...
        /* خط خالی در حالت multiline = اجرا */
        if(!strlen(line)){
        ...
        if(st == -1 && multi) {
            /* در حالت multiline خطا گرفت - اجرا کن تا خطا نشون بده */
            strncpy(ml, test, sizeof(ml)-1);
            exec(ml);
            multi=0; ml[0]='\0';
            hist_add(line);
